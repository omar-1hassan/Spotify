...
final class APICaller {
    ...
    public func removerTrackFromPlaylist(track: AudioTrack, playlist: Playlist, completion: @escaping(Bool) -> Void){
    ...
    public func getGategoryPlaylist(category: Category, completion: @escaping(Result<[Playlist], Error>) -> Void) {
        ...
        createRequest(with: URL(string: Constants.baseAPIURL + "/search?limit=10&type=album,artist,playlist,track&q=\(query.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")"),
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                ...
                do {
                    ...
                    var searchResuls: [SearchResult] = []
                    searchResuls.append(contentsOf: result.tracks.items.compactMap({ .track(model: $0) }))
                    searchResuls.append(contentsOf: result.albums.items.compactMap({ .album(model: $0) }))
                    searchResuls.append(contentsOf: result.artists.items.compactMap({ .artist(model: $0) }))
                    searchResuls.append(contentsOf: result.playlists.items.compactMap({ .playlist(model: $0) }))
