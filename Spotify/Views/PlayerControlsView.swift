...
final class PlayerControlsView: UIView {
    ...
    private let playPauseButton: UIButton = {
        let button = UIButton()
        button.tintColor = .label
        let image = UIImage(systemName: "pause", withConfiguration: UIImage.SymbolConfiguration(pointSize: 37, weight: .regular))
        button.setImage(image, for: .normal)
        return button
    }()
    ...
    @objc private func didTapBack(){
        delegate?.PlayerControlsViewDidTapBackwadButton(self)
    }
    ...
    @objc private func didTapPlayPause(){
        ...
        //Update the icon
        let pause = UIImage(systemName: "pause", withConfiguration: UIImage.SymbolConfiguration(pointSize: 37, weight: .regular))
